...
void _qsort(int *line, int left, int right); // Функция быстрой сортировки массива чисел типа int. Принимает массив, индекс левого и правого элемента массива

int main()
{
    int **matrix = (int **) malloc(sizeof(int *) * ROW); // матрица 
    int row = 0, column = 0; // строка, колонка
    
    if (matrix == NULL) // Если память не выделена
    {
        printf("Error while allocating memory");
        return 1; // Ошибка, выйти из программы 
    }
    for (int tmp = 0; tmp < ROW; tmp++) // Для каждой строки матрицы
    {
        matrix[tmp] = (int *)malloc(COLUMN * sizeof(int)); // Выделить память под колонки
        if (matrix[tmp] == NULL) // Если память не выделена
        {
            printf("Error while allocating memory");
            while (tmp > 0) 
                free(matrix[--tmp]); // Освободить ранее выделенную память
            free(matrix);
            return 1; // Ошибка, выйти из программы
        }
    }
    ...
    for (int tmp = 0; tmp < ROW; tmp++) // Для каждой строки матрицы 
        free(matrix[tmp]); // Освободить память для строки
    free(matrix); // Освободить память для матрицы
